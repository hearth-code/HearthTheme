...
const DEFAULT_MODEL_SOURCES = Object.freeze({
  activeScheme: 'injected:activeScheme',
  scheme: 'injected:scheme',
  philosophy: 'injected:philosophy',
  taxonomy: 'injected:taxonomy',
  foundation: 'injected:foundation',
  surfaceRules: 'injected:surfaceRules',
  guidanceRules: 'injected:guidanceRules',
  terminalRules: 'injected:terminalRules',
  interfaceRules: 'injected:interfaceRules',
  interactionRules: 'injected:interactionRules',
  feedbackRules: 'injected:feedbackRules',
  semanticRules: 'injected:semanticRules',
  variantKnobs: 'injected:variantKnobs',
  variantProfiles: 'injected:variantProfiles',
  adapters: 'injected:adapters',
  variants: 'injected:variants',
  tuning: 'injected:tuning',
  semanticSnapshot: 'injected:semanticSnapshot',
})

function resolveModelSources(sources = null) {
  return {
...
function requireModelInput(inputs, key) {
  if (inputs?.[key] == null) {
    throw new Error(`buildColorLanguageModel: missing injected input "${key}"`)
  }
  return inputs[key]
}

function buildSemanticSnapshotDocument(palette, sources) {
  return {
    schemaVersion: 3,
    generated: true,
    generatedFrom: {
      activeScheme: sources.activeScheme,
      scheme: sources.scheme,
      philosophy: sources.philosophy,
      taxonomy: sources.taxonomy,
      foundation: sources.foundation,
      surfaceRules: sources.surfaceRules,
      terminalRules: sources.terminalRules,
      interfaceRules: sources.interfaceRules,
      interactionRules: sources.interactionRules,
      feedbackRules: sources.feedbackRules,
      semanticRules: sources.semanticRules,
      variantKnobs: sources.variantKnobs,
      variantProfiles: sources.variantProfiles,
      variants: sources.variants,
      adapters: sources.adapters,
      calibration: sources.tuning,
    },
    roles: palette,
  }
}
...
export function buildColorLanguageModel({
  domain = defaultThemeDomain,
  inputs = null,
  overrides = null,
  sources = null,
} = {}) {
  // Pure, browser-safe model build: callers provide every source document and
  // adapter contract up front. `overrides` remains a convenient in-memory patch
  // layer over those injected inputs for live playground edits.
  const modelInputs = {
    ...(inputs || {}),
    ...(overrides || {}),
  }
  const modelSources = resolveModelSources(sources)
  const activeScheme = requireModelInput(modelInputs, 'activeScheme')
  const scheme = requireModelInput(modelInputs, 'scheme')
  const taxonomy = requireModelInput(modelInputs, 'taxonomy')
  const variantSpec = requireModelInput(modelInputs, 'variants')
  const adapters = requireModelInput(modelInputs, 'adapters')
  const surfaceAdapters = requireModelInput(modelInputs, 'surfaceAdapters')
  const guidanceAdapters = requireModelInput(modelInputs, 'guidanceAdapters')
  const terminalAdapters = requireModelInput(modelInputs, 'terminalAdapters')
  const interfaceAdapters = requireModelInput(modelInputs, 'interfaceAdapters')
  const interactionAdapters = requireModelInput(modelInputs, 'interactionAdapters')
  const feedbackAdapters = requireModelInput(modelInputs, 'feedbackAdapters')
  const foundation = requireModelInput(modelInputs, 'foundation')
  const rawSurfaceRules = requireModelInput(modelInputs, 'surfaceRules')
  const rawGuidanceRules = requireModelInput(modelInputs, 'guidanceRules')
  const rawTerminalRules = requireModelInput(modelInputs, 'terminalRules')
  const rawInterfaceRules = requireModelInput(modelInputs, 'interfaceRules')
  const rawInteractionRules = requireModelInput(modelInputs, 'interactionRules')
  const rawFeedbackRules = requireModelInput(modelInputs, 'feedbackRules')
  const semanticRules = requireModelInput(modelInputs, 'semanticRules')
  const variantKnobs = requireModelInput(modelInputs, 'variantKnobs')
  const variantProfiles = requireModelInput(modelInputs, 'variantProfiles')
  ...
  const semanticSnapshot = buildSemanticSnapshotDocument(palette, modelSources)
  ...
  const model = {
    sources: {
      activeScheme: modelSources.activeScheme,
      scheme: modelSources.scheme,
      philosophy: modelSources.philosophy,
      taxonomy: modelSources.taxonomy,
      foundation: modelSources.foundation,
      surfaceRules: modelSources.surfaceRules,
      guidanceRules: modelSources.guidanceRules,
      terminalRules: modelSources.terminalRules,
      interfaceRules: modelSources.interfaceRules,
      interactionRules: modelSources.interactionRules,
      feedbackRules: modelSources.feedbackRules,
      semanticRules: modelSources.semanticRules,
      variantKnobs: modelSources.variantKnobs,
      variantProfiles: modelSources.variantProfiles,
      adapters: modelSources.adapters,
      variants: modelSources.variants,
      tuning: modelSources.tuning,
      semanticSnapshot: modelSources.semanticSnapshot,
    },
    activeScheme,
    scheme,
    taxonomy,
