...
function buildAppThemeCss() {
  ...
  // The @settings comment sits up top for discoverability, but the consuming CSS
  // goes AFTER the canonical .theme-dark/.theme-light blocks: the accent presets
  // redefine --interactive-accent etc., and several tools (the screenshot renderer,
  // audits) read theme colors by matching the FIRST `.theme-<mode> {` block. Keeping
  // overrides last means those parsers always see the real defaults, while the
  // overrides still win at runtime on specificity.
  return [
